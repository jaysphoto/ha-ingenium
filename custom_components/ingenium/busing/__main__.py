...
async def main(host: str, port: int, raw_msg: str | None, bus_init=bool) -> None:
    ...
    if raw_msg is not None:
        ...
        asyncio.create_task(client.send_message_raw(
            ...
            command=4, origin=-1, destination=1, data1=1, data2=1)) -> ffff0001040205 # ENABLE OUTPUT 5 SWITCH DEVICE (Actuador)
                Received response: {'raw': 'fefe01fefe00010205', 'command': 1, 'origin': 65278, 'destination': 1, 'data1': 2, 'data2': 5}
            command=4, origin=-1, destination=1, data1=1, data2=1)) -> ffff000104020d # DISABLE OUTPUT 5 SWITCH DEVICE (Actuador)
